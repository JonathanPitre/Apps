...
$appChannel = "6.0" # or STS (7.X), LTS (8.X)
...
$appMajorVersion = $appVersion.SubString(0, 3)
$appURL = $Evergreen.URI
$appSetup = Split-Path -Path $appURL -Leaf
[boolean]$IsAppInstalled = [boolean](Get-InstalledApplication -Name "$appVendor $appName - $appMajorVersion.* ($appArchitecture)" -WildCard)
$appInstalledVersion = ((Get-InstalledApplication -Name "$appVendor $appName - $appMajorVersion.* ($appArchitecture)" -WildCard).DisplayVersion) | Sort-Object -Descending | Select-Object -First 1
$appInstalledVersion = $appInstalledVersion.Substring(0, 6)
$appUninstallString = ((Get-InstalledApplication -Name "$appVendor $appName - $appMajorVersion.* ($appArchitecture)" -WildCard).UninstallString).Split("")
...
If ([version]$appVersion -gt [version]$appInstalledVersion)
{
    ...
    Remove-MSIApplications -Name "$appVendor $appName - $appMajorVersion.* ($appArchitecture)" -Wildcard

    # Download latest setup file(s)
    If (-Not(Test-Path -Path $appScriptPath\$appVersion\$appSetup))
    {
        Write-Log -Message "Downloading $appVendor $appName $appVersion $appChannel $appArchitecture..." -Severity 1 -LogType CMTrace -WriteHost $True
    ...
    Write-Log -Message "Installing $appVendor $appName $appVersion $appChannel $appArchitecture..." -Severity 1 -LogType CMTrace -WriteHost $True
    ...
    Write-Log -Message "$appVendor $appName $appVersion $appChannel $appArchitecture was installed successfully!" -Severity 1 -LogType CMTrace -WriteHost $True
}
Else
{
    Write-Log -Message "$appVendor $appName $appInstalledVersion $appChannel $appArchitecture is already installed." -Severity 1 -LogType CMTrace -WriteHost $True
}