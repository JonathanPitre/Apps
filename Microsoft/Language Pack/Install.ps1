...
If ($envOSName -like "*Windows 10*" -or $envOSName -like "*Windows 11*" -or $envOSName -like "*Windows Server 2019*" -or $envOSName -like "*Windows Server 2022*")
{
    $installedLanguages = (Get-InstalledLanguage).LanguageId

    If ($installedLanguages -notcontains $languagePacks)
    {
        ...
        # Install language pack(s) - # https://learn.microsoft.com/en-us/powershell/module/languagepackmanagement/install-language?view=windowsserver2019-ps
        Write-Log -Message "Installing $languagePacks language pack(s)..." -Severity 1 -LogType CMTrace -WriteHost $True
        Install-Language -Language $languagePacks -CopyToSettings
        Write-Log -Message "$languagePacks language pack(s) were successfully installed!" -Severity 1 -LogType CMTrace -WriteHost $True
        Write-Log -Message "Setting default Language to: $defaultLanguage" -Severity 1 -LogType CMTrace -WriteHost $True
        # Set default language
        Set-SystemPreferredUILanguage $defaultLanguage
    }
    Else
    {
        Write-Log -Message "$languagePacks language pack(s) are already installed!" -Severity 1 -LogType CMTrace -WriteHost $True
    }
}
Else
{
