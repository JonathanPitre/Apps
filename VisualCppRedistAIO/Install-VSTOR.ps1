...
Function Get-MicrosoftVisualCPlusPlusVersion
{
    ...
    [CmdletBinding()]
    Param ()
    $DownloadURL = "https://github.com/abbodi1406/vcredist"
    ...
    Finally
    {
        $RegEx = "(Visual Studio 2010 Tools for Office Runtime \(x86\/x64\)\<br\>)\n((?:\d+\.)+(?:\d+))"
...
$appName2 = "Microsoft Visual Studio 2010 Tools for Office Runtime"
...
If ([version]$appVersion -gt [version]$appInstalledVersion)
{
    ...
    If (-Not(Test-Path -Path $appScriptPath\$appSetupVersion\$appSetup))
    {
        ...
        Expand-Archive -Path $appZip -DestinationPath $appScriptPath\$appSetupVersion
    ...
    Write-Log -Message "Installing $appName2 $appVersion..." -Severity 1 -LogType CMTrace -WriteHost $True
    Execute-Process -Path .\$appSetup -Parameters $appInstallParameters

    # Go back to the parent folder
    Set-Location ..

    Write-Log -Message "$appName2 $appVersion was installed successfully!" -Severity 1 -LogType CMTrace -WriteHost $True
}
Else
{
    Write-Log -Message "$appName2 $appVersion is already installed." -Severity 1 -LogType CMTrace -WriteHost $True
}