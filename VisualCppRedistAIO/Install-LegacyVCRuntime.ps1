...
If ([version]$appVersion -gt [version]$appInstalledVersion)
{
    ...
    Write-Log -Message "Installing $appName2 ..." -Severity 1 -LogType CMTrace -WriteHost $True
    Execute-Process -Path .\$appSetup -Parameters $appInstallParameters

    # Go back to the parent folder
    Set-Location ..

    Write-Log -Message "$appName2 were installed successfully!" -Severity 1 -LogType CMTrace -WriteHost $True
}
Else
{
    Write-Log -Message "$appName2 are already installed." -Severity 1 -LogType CMTrace -WriteHost $True
}