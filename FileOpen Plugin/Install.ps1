...
$appName = "FileOpenPlugin"
$appName2 = "FileOpen Client"
$appArchitecture = "x64"
$appProcesses = @("FileOpenBroker64", "FileOpenManager64")
$appInstallParameters = "/QB"
$Evergreen = Get-NevergreenApp -Name $appName | Where-Object { $_.Type -eq "msi" }
$appVersion = $Evergreen.Version
$appURL = $Evergreen.URI
$appSetup = Split-Path -Path $appURL -Leaf
$appDestination = "$Env:ProgramFiles\FileOpen"
[boolean]$IsAppInstalled = [boolean](Get-InstalledApplication -Name "$appName2 ($appArchitecture)")
$appInstalledVersion = ((Get-InstalledApplication -Name "$appName2 ($appArchitecture)").DisplayVersion)
$appInstalledVersion = $appInstalledVersion -replace '^.*(?=.{3}$)'
...
If ($appVersion -gt $appInstalledVersion)
{
    ...
    If ($IsAppInstalled)
    {
        ...
        Remove-MSIApplications -Name $appName2 -Parameters $appInstallParameters
    }
    ...
    # Remove startup item
    #Remove-RegistryKey -Key "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Run" -Name "FileOpenBroker"
